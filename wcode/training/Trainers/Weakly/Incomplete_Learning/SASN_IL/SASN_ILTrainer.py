...
from wcode.training.Trainers.Weakly.Incomplete_Learning.SASN_IL.models import (
    MeanTeacher,
)
from wcode.training.Trainers.Weakly.Incomplete_Learning.SASN_IL.early_learning_detection import (
    find_t0,
)
from wcode.training.Trainers.Weakly.Incomplete_Learning.SASN_IL.label_correction import (
    correction_by_fn_estim,
)
...
class SASN_ILTrainer(PatchBasedTrainer):
    def __init__(
        self,
        ...
        hyperparams_name = "alpha_{}_tau_{}_stage1_epochs_{}_awce_beta_{}_improved_weight_{}_implement_type_{}".format(
            self.alpha,
            self.tau,
            self.stage1_epochs,
            self.awce_beta,
        ...
        if self.resume_flag:
            self.save_every = 1
            selected_I_r = os.path.join(self.logs_output_folder, "selected_t0.pth")
            assert os.path.isfile(
                selected_I_r
            ), "File selected_t0.pth is not found. You should set resume_flag to False to run stage-1 first."
            self.initialize()
            load_pretrained_weights(
                self.network.teacher,
                os.path.join(self.logs_output_folder, "selected_t0.pth"),
                load_all=True,
                verbose=True,
            )
            load_pretrained_weights(
                self.network.student,
                os.path.join(self.logs_output_folder, "selected_student.pth"),
                load_all=True,
                verbose=True,
            )
            ...
            self.IoU_on_noisy_label = checkpoint["IoU_on_noisy_label"]
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.dice_loss = TverskyLoss(reduction="batch")
            ...
            if self.do_deep_supervision:
                ...
                self.dice_loss = self._build_deep_supervision_loss_object(
                    self.dice_loss
                )
    ...
    def get_networks(self, network_settings):
        if not self.resume_flag:
            ...
            return MeanTeacher(network_settings, alpha=self.alpha)
    ...
    def get_optimizers(self):
        ...
        #     self.network.student.parameters() if self.resume_flag else self.network.parameters(),
        #     self.base_lr,
        #     weight_decay=self.weight_decay,
        #     momentum=0.9,
        #     nesterov=True,
    ...
    def epoch_start(self):
        # if not self.resume_flag:
        #     # stage-1
        #     if self.saved_num_iters is None:
        #         self.saved_num_iters = self.tr_iterations_per_epoch
        #     self.tr_iterations_per_epoch = 250
        # else:
        #     if self.saved_num_iters is not None:
        #         self.tr_iterations_per_epoch = self.saved_num_iters
    ...
    def epoch_end(self, epoch):
        ...
        if self.current_epoch >= self.stage1_epochs:
            t0 = find_t0(self.IoU_on_noisy_label, self.tau)
        else:
            t0 = 0
        if t0 > 0 and not self.resume_flag:
            # Reload the check point at I_r.
            ## find the checkpoint
            self.resume_flag = True
            epochs_find = int(t0 / self.save_every) * self.save_every
            self.print_to_log_file(
            ...
            os.rename(
                os.path.join(
                    self.logs_output_folder,
                    "checkpoint_Epoch{}.pth".format(self.stage1_epochs),
                ),
                os.path.join(self.logs_output_folder, "selected_student.pth"),
            )
            os.rename(
    ...
    def train_epoch_start(self):
        ...
        self.print_to_log_file(
            f"learning rate: {[np.round(p['lr'], decimals=5) for p in self.optimizer.param_groups]}"
        )
        self.logger.log(
    ...
    def train_step(self, batch):
        ...
        with (
            ...
            if self.resume_flag:
                outputs = self.network(images, train_flag=True, weak_aug=True)
                ...
                student_output = outputs["pred"]

            if self.implement_type == "original":
                # original implementation
                if self.resume_flag:
                    # performing label correction
                    refine_labels = correction_by_fn_estim(
                        teacher_output.detach(), labels
                    )

                    # Compute image-level weight
                    with torch.no_grad():
                        preds_T_soft = teacher_output.detach().softmax(1)
                        confidence_T, _ = preds_T_soft.max(1)
                        entropy = 1 - (
                            -preds_T_soft * preds_T_soft.log().clip(min=1e-6)
                        ).sum(1) / math.log(2)
                        rho = (confidence_T * entropy).mean(
                            [i for i in range(1, entropy.ndim)]
                        )  # [bs]

                    # Compute Loss
                    dice_b = self.dice_loss(student_output, refine_labels)
                    ce_b = self.ce_loss(student_output, refine_labels)
                    ce_b = ce_b.mean([i for i in range(1, ce_b.ndim)])
                    l = (rho * 0.5 * (dice_b + ce_b)).sum()
                else:
                    # Compute Loss
    ...
    def validation_for_train_step(self, batch):
        # images in (b, c, (z,) y, x) and labels in (b, 1, (z,) y, x) or list object if do deep supervision
        images = batch["image"]
        labels = batch["label"]
        ...
        with (
            ...
            outputs = self.network(images, train_flag=True)["student_out"]
            if isinstance(outputs, dict):
                outputs = outputs["pred"]
            del images
            l = self.val_loss(outputs, labels)
    ...
    def save_checkpoint(self, filename: str) -> None:
        if not self.disable_checkpointing:
            ...
            checkpoint = {
                ...
                "IoU_on_noisy_label": self.IoU_on_noisy_label,
            }
