...
from wcode.training.loss.EntropyLoss import MixCrossEntropyLoss, RobustCrossEntropyLoss
...
class DenseDeSCOTrainer(PatchBasedTrainer):
    """
    ...
    def __init__(
        ...
        hyperparams_name = "dropout_n_{}_consistency_weight_{}_threshold_{}_rampup_epoch_{}_implement_type_{}".format(
            self.dropout_n,
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.cps_loss = RobustCrossEntropyLoss(reduction="none")
            ...
            if self.do_deep_supervision:
                ...
                self.cps_loss = self._build_deep_supervision_loss_object(self.cps_loss)
    ...
    def train_step(self, batch):
        ...
        with (
            ...
            if self.implement_type == "improved":
                sup_loss = 0.5 * (
                    self.tversky_loss(logits1, labels)
                    + self.mixce_loss(logits1, labels)
                    + self.tversky_loss(logits2, labels)
                    + self.mixce_loss(logits2, labels)
                )
            elif self.implement_type == "original":
                sup_loss = 0.5 * (
                    self.train_loss(logits1, labels) + self.train_loss(logits2, labels)
                )
            else:
