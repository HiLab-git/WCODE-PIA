...
from wcode.training.Trainers.Weakly.Incomplete_Learning.DBDMP.models import DBUNet
...
class DBDMPTrainer(PatchBasedTrainer):
    """
    This is a refined and simplified version.
    ...
    def __init__(
        ...
        self.tau = tau
        self.lambda_for_plabel = lambda_for_plabel
        self.rampup_epoch = rampup_epoch
        hyperparams_name = "tversky_alpha_{}_sce_alpha_{}_tau_{}_lambda_for_plabel_{}_rampup_epoch_{}".format(
            self.tversky_alpha,
            self.sce_alpha,
            self.tau,
            self.lambda_for_plabel,
            self.rampup_epoch,
        )
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.ramp = ramps(
                ...
                start_value=0,
                end_value=self.lambda_for_plabel,
                mode="sigmoid",
            )
            ...
            self.SCE_loss = SymmetricCrossEntropyLoss(
                alpha=self.sce_alpha, beta=1.0, eps=1e-7
            )
            self.PCE_loss = RobustCrossEntropyLoss(ignore_index=0)
            self.KLCE_loss = KL_CE_loss()
            ...
            if self.do_deep_supervision:
                ...
                self.SCE_loss = self._build_deep_supervision_loss_object(self.SCE_loss)
                self.PCE_loss = self._build_deep_supervision_loss_object(self.PCE_loss)
                self.KLCE_loss = self._build_deep_supervision_loss_object(
                    self.KLCE_loss
                )
    ...
    def get_networks(self, network_settings):
        return DBUNet(network_settings)
    ...
    def train_step(self, batch):
        ...
        with (
            ...
            beta = torch.randn(
                main_logits.shape[0],
                *[1 for _ in range(len(main_logits.shape) - 1)],
                device=self.device,
            )
            # if isinstance(main_logits, (tuple, list)):
            if False:
                # I write the code for deep supervision, but pce can be nan while doing so.
                # So I write but do not use the following code.
                main_soft_label = []
                aux_soft_label = []
                pseudo_soft_label = []
                for i in range(len(main_logits)):
                    main_soft_label.append(torch.softmax(main_logits[i], dim=1))
                    aux_soft_label.append(torch.softmax(aux_logits[i], dim=1))
                    # pseudo_soft_label.append(
                    #     torch.softmax(
                    #         (
                    #             beta * main_soft_label.detach()
                    #             + (1.0 - beta) * aux_soft_label.detach()
                    #         )
                    #         / self.tau,
                    #         dim=1,
                    #     )
                    ...
                    # mix original label and the pseudo label
                    pseudo_soft_label[i][:, 0][labels[i][:, 0] == 1] = 1e-4
                    pseudo_soft_label[i][:, 1][labels[i][:, 0] == 1] = 1.0 - 1e-4
            else:
                main_soft_label = torch.softmax(main_logits, dim=1)
                aux_soft_label = torch.softmax(aux_logits, dim=1)
                pseudo_soft_label = torch.softmax(
                    (
                        beta * main_soft_label.detach()
                        + (1.0 - beta) * aux_soft_label.detach()
                    )
                ...
                # mix original label and the pseudo label
                pseudo_soft_label[:, 0][labels[:, 0] == 1] = 1e-4
                pseudo_soft_label[:, 1][labels[:, 0] == 1] = 1.0 - 1e-4

            # compute loss for pseudo label
            loss_by_pseudo_label = 0.5 * (
                self.KLCE_loss(main_logits, aux_logits, pseudo_soft_label)
                + self.KLCE_loss(aux_logits, main_logits, pseudo_soft_label)
            )

            # update the weight of the loss of pseudo label
            weight_pseudo = self.ramp.get_value(self.current_epoch)

            # Compute Loss
            l = loss_by_label + weight_pseudo * loss_by_pseudo_label
