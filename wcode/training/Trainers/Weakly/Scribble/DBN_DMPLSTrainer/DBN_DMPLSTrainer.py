...
from wcode.training.Trainers.Weakly.Scribble.DBN_DMPLSTrainer.DBUNet import DBUNet
...
class DBN_DMPLSTrainer(PatchBasedTrainer):
    """
    ...
    def __init__(
        ...
        hyperparams_name = "lambda_for_plabel_{}_rampup_epoch_{}_w_class_{}".format(
            self.lambda_for_plabel, self.rampup_epoch, self.w_class
        )
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            i = np.arange(self.rampup_epoch)
            rampup_pseudo_label = 1 - i / (self.rampup_epoch - 1)
            self.weight_pseudo_label = self.lambda_for_plabel * np.exp(
                -5.0 * rampup_pseudo_label * rampup_pseudo_label
            )
            ...
            self.PCE_loss = RobustCrossEntropyLoss(
                weight=(
                    torch.tensor(self.w_class, device=self.device)
                    if self.w_class
                    else None
                ),
    ...
    def train_step(self, batch):
        ...
        if self.preprocess_config == "3d":
            ...
            raise ValueError(
                "Training dimension only support 2 or 3. But get {}.".format(
        ...
        with (
            ...
            # PCE Loss
            pce_loss = 0.5 * (
                self.PCE_loss(main_logits, labels) + self.PCE_loss(aux_logits, labels)
            )
            ...
            mixed_prob = (
                beta * main_soft_label.detach() + (1.0 - beta) * aux_soft_label.detach()
            )
            pseudo_supervision = torch.argmax(mixed_prob, dim=1)

            # pseudo_label loss
            loss_by_pseudo_label = 0.5 * (
                self.Tversky_loss(main_logits, pseudo_supervision)
                + self.Tversky_loss(aux_logits, pseudo_supervision)
            )

            # update the weight of the loss of pseudo label
            if self.current_epoch < len(self.weight_pseudo_label):
                # weight_a = weight_label[self.current_epoch]
                weight_pseudo = self.weight_pseudo_label[self.current_epoch]
            else:
                weight_pseudo = self.lambda_for_plabel

            l = pce_loss + weight_pseudo * loss_by_pseudo_label
